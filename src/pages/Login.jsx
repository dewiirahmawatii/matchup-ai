...
export default function Login() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (tab === 'login') {
      // Simulate login success -> redirect to Complete Profile (next page in user flow)
      navigate('/complete-profile');
    } else {
      // Simulate registration success -> redirect to Complete Profile
      navigate('/complete-profile');
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col relative pt-20 pb-12 px-margin-mobile">
      ...
      <div className="w-full max-w-md mx-auto my-auto" id="auth-container">
        <div className="auth-card glass-panel rounded-[32px] p-8 border border-outline-variant/40">
          {/* Tabs */}
          <div className="flex p-1 bg-surface-container-high rounded-full mb-8">
            <button 
              className={`flex-1 py-2 rounded-full font-label-sm text-label-sm transition-all duration-300 ${tab === 'login' ? 'bg-primary-container text-on-primary-container shadow-sm' : 'text-on-surface-variant hover:bg-surface-container-highest'}`}
              onClick={() => setTab('login')}
            >
              Login
            </button>
            <button 
              className={`flex-1 py-2 rounded-full font-label-sm text-label-sm transition-all duration-300 ${tab === 'register' ? 'bg-primary-container text-on-primary-container shadow-sm' : 'text-on-surface-variant hover:bg-surface-container-highest'}`}
          ...
          <div className="mb-8" id="auth-header">
            <h2 className="font-headline-lg-mobile text-headline-lg-mobile text-on-surface mb-2">
              {tab === 'login' ? 'Welcome back' : 'Join MatchUp AI'}
            </h2>
            <p className="font-body-md text-body-md text-on-surface-variant">
              {tab === 'login' ? 'Sign in to continue your career journey.' : 'Start your journey to a better career today.'}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <button onClick={() => navigate('/complete-profile')} className="h-[56px] flex items-center justify-center gap-3 border border-outline-variant rounded-xl hover:bg-surface-container-low transition-colors active:scale-95 duration-150">
